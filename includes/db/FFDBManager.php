...
class FFDBManager extends LADBManager{
	...
	public function save_sources_settings(){
		if (isset($_POST['model'])){
			$model = $_POST['model'];
			$model['id'] = 1; // DON'T DELETE, ID is always 1, this is needed to detect if it model was saved
	...
	public function  ff_save_settings_fn () {
		try{
			...
			FFDB::commit();
			echo json_encode( array('settings' => $settings, 'fb_extended_token' => $extendedToken, 'activated' => false) ); // return serialized string back to client
	...
	private function clean_cache($options) {
		...
		} else if (trim($fb['facebook_access_token']) == '' && trim($fb['facebook_app_id']) == '' && trim($fb['facebook_app_secret']) == ''){
				$this->cleanByFeedType('facebook');
				$force_load_cache = true;
				$facebook_changed = true;
		}
