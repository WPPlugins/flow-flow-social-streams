...
class FFFacebookCacheManager implements LAFacebookCacheManager{
	...
	protected function getStoredToken() {
		...
		$access_token_transient = $this->getOption($at);
		if (false !== $access_token_transient && !empty($access_token_transient)){
	...
	protected function refreshToken( $oldToken ) {
		...
		if (false !== $response['response']){
			...
			$expires = isset($json->expires_in) ? (int)$json->expires_in: time() + 2629743*2;
			return array($json->access_token, $expires, null);
		}
