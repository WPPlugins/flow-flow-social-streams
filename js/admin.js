...
//    "click .stream-streams__item": "showPreview",
      ...
      "change [id^=gc-style]" : "previewChangeStyle",
      "change [id^=theme-classic]" : "previewChangeTheme"
    },
    ...
    configDesign: function () {
      ...
      this.$el.find('select[id*=cmeta], .design-step-2 select[id*=align], .layout-compact select[id*=compact-style]').change();
      this.$el.find('[id^=stream-layout]').trigger('change');
      this.$el.find('[id^=theme-classic]').trigger('change');
      this.$el.find('[id^=gc-style]').trigger('change');
    ...
    previewChangeStyle: function (e) {
      debugger
      var val = e.target.value;
      var $preview = $(e.target).closest('.section').find('.preview .ff-stream-wrapper');
      var cls = $preview.attr( 'class' );
      debugger
      if (/flat/.test(cls)) {
        this.revert($preview);
